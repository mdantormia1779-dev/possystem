...
const INITIAL_BRANDS: BrandItem[] = [
  { id: 'b-1', brandName: 'MOTO WOLF', shortDescription: '' },
  { id: 'b-2', brandName: 'Robin', shortDescription: '' },
  { id: 'b-3', brandName: 'WD', shortDescription: '' },
];
...
export default function BrandsPage() {
  ...
  const handleOpenEditModal = (item: BrandItem) => {
  ...
  const handleExportCSV = () => {
    ...
    const rows = filteredBrands.map((b) => [b.brandName, b.shortDescription || '']);
    const csvContent =
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-7xl mx-auto select-none min-h-screen">
      ...
      <div className="rounded-2xl sm:rounded-3xl bg-[#120e34]/85 border border-white/10 shadow-xl backdrop-blur-xl p-6 sm:p-7 space-y-5">
        ...
        {/* Table (matching media_1789383598591.png) */}
        <div className="overflow-x-auto rounded-xl border border-white/10">
          <table className="w-full text-left text-xs whitespace-nowrap border-collapse">
            <thead>
              <tr className="bg-white/5 text-slate-300 font-semibold border-b border-white/10">
                <th className="px-6 py-3 min-w-[200px]">Brands</th>
                <th className="px-6 py-3 min-w-[240px]">Note</th>
              ...
              {displayedBrands.length === 0 ? (
                ...
                displayedBrands.map((item) => (
                  <tr key={item.id} className="hover:bg-white/[0.02] transition-colors">
                    <td className="px-6 py-3.5 font-medium text-white">{item.brandName}</td>
                    <td className="px-6 py-3.5 text-slate-400">{item.shortDescription || ''}</td>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs text-slate-400 pt-2">
          <div>
            Showing{' '}
            {filteredBrands.length === 0 ? 0 : (currentPage - 1) * entriesPerPage + 1} to{' '}
      ...
      {/* Add / Edit Brand Modal (matching media_1789383612779.png) */}
      {isModalOpen && (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-150">
          ...
          <div className="relative w-full max-w-md bg-[#120e34] rounded-2xl p-6 border border-white/15 shadow-2xl z-10 text-slate-200">
            ...
            <p className="text-xs text-slate-400 leading-relaxed">
              ...
              <strong className="text-white">&quot;{brandToDelete.brandName}&quot;</strong>? This action
              cannot be undone.
            </p>
