...
export interface WarrantyItem {
  ...
  durationType: 'Days' | 'Months' | 'Years' | 'Please Select';
}
...
export default function WarrantiesPage() {
  ...
  const [durationType, setDurationType] = useState<'Days' | 'Months' | 'Years' | 'Please Select'>('Please Select');
  const [modalError, setModalError] = useState('');

  // Delete modal state
  const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false);
  const [itemToDelete, setItemToDelete] = useState<WarrantyItem | null>(null);

  // Filtered warranties
  const filteredWarranties = useMemo(() => {
    return warranties.filter((w) => {
      const q = searchQuery.toLowerCase();
      const dur = `${w.durationValue} ${w.durationType}`.toLowerCase();
      return (
  ...
  const totalPages = Math.max(1, Math.ceil(filteredWarranties.length / entriesPerPage));
  const displayedWarranties = useMemo(() => {
    const start = (currentPage - 1) * entriesPerPage;
    return filteredWarranties.slice(start, start + entriesPerPage);
  }, [filteredWarranties, currentPage, entriesPerPage]);
  ...
  const handleOpenEditModal = (item: WarrantyItem) => {
    ...
    setDurationType(item.durationType);
    setModalError('');
    setIsAddEditModalOpen(true);
  };
  ...
  const handleSaveModal = (e: React.FormEvent) => {
    ...
    if (durationType === 'Please Select') {
      setModalError('Please select a duration unit (Days, Months, Years).');
      return;
    }
  ...
  const handleDeleteConfirm = () => {
    if (itemToDelete) {
      setWarranties((prev) => prev.filter((w) => w.id !== itemToDelete.id));
  ...
  const handleExportCSV = () => {
    ...
    const rows = filteredWarranties.map((w) => [
      w.name,
      w.description || '',
      `${w.durationValue} ${w.durationType}`,
    ]);
    const csvContent =
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-7xl mx-auto select-none min-h-screen">
      ...
      <div className="rounded-2xl sm:rounded-3xl bg-[#120e34]/85 border border-white/10 shadow-xl backdrop-blur-xl p-6 sm:p-7 space-y-5">
        ...
        {/* Table (matching media_1789383922713.png) */}
          ...
          <table className="w-full text-left text-xs whitespace-nowrap border-collapse">
            <thead>
              <tr className="bg-white/5 text-slate-300 font-semibold border-b border-white/10">
                ...
                <th className="px-6 py-3 min-w-[260px]">Description</th>
                <th className="px-6 py-3 min-w-[180px]">Duration</th>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs text-slate-400 pt-2">
          <div>
            Showing{' '}
            {filteredWarranties.length === 0 ? 0 : (currentPage - 1) * entriesPerPage + 1} to{' '}
            {Math.min(currentPage * entriesPerPage, filteredWarranties.length)} of{' '}
      ...
      {/* Add / Edit Warranty Modal (matching media_1789383938654.png) */}
      {isAddEditModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-150">
          ...
          <div className="relative w-full max-w-lg bg-[#120e34] rounded-2xl sm:rounded-3xl shadow-2xl border border-white/15 overflow-hidden z-10 flex flex-col text-slate-200 animate-in zoom-in-95 duration-150">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-white/10 bg-[#0e0a2b]">
              <h2 className="text-base sm:text-lg font-bold text-white tracking-wide">
                {editingItem ? 'Edit Warranty' : 'Add Warranty'}
              </h2>
              <button
                type="button"
                onClick={() => setIsAddEditModalOpen(false)}
            ...
            <form onSubmit={handleSaveModal} className="p-6 space-y-4 text-xs font-sans">
              ...
              {/* Duration:* (input + dropdown side by side matching screenshot) */}
              <div>
                <label className="block text-slate-300 font-semibold mb-1.5">
                  Duration:<span className="text-rose-400 ml-0.5">*</span>
                </label>
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <select
                    value={durationType}
                    onChange={(e) => {
                      setDurationType(e.target.value as any);
                      if (modalError) setModalError('');
                    }}
