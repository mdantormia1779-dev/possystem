...
export default function AddContactModal({ isOpen, onClose, onSave }: AddContactModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const displayName = contactType === 'Business' && businessName.trim()
      ? businessName.trim()
      : (name.trim() || `Contact ${mobile.trim()}`);

    const newContact: NewContactData = {
      id: String(Date.now()),
      contactType,
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-3 sm:p-5 overflow-y-auto bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
      <div className="relative w-full max-w-4xl bg-[#120e34] border border-white/15 rounded-2xl sm:rounded-3xl shadow-2xl text-slate-200 overflow-hidden my-auto max-h-[92vh] flex flex-col font-sans">
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-5 sm:px-7 py-6 space-y-6 scrollbar-thin scrollbar-thumb-white/15 text-xs text-slate-300">
          ...
          {/* Radio Buttons: Individual vs Business */}
          <div className="flex items-center gap-6">
            <label className="inline-flex items-center gap-2.5 cursor-pointer font-semibold text-slate-200">
              <input
                type="radio"
                name="contactType"
                value="Individual"
            ...
            <div>
              ...
              <div className="relative flex items-center">
                ...
                <input
                  ...
                  placeholder="Alternate contact num"
                  className="w-full bg-[#08051e] border border-white/15 rounded-xl pl-10 pr-3.5 py-2.5 text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500/60 shadow-inner"
                />
              </div>
            </div>
          ...
          {showMoreInfo && (
            <div className="p-5 rounded-2xl bg-[#0c0827]/70 border border-white/10 space-y-4 animate-in fade-in slide-in-from-top-2 duration-200">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block font-semibold text-slate-300 mb-1.5">Address Line 2:</label>
                  <input
                    type="text"
                    value={addressLine2}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="City"
                    className="w-full bg-[#08051e] border border-white/15 rounded-xl px-3 py-2 text-white placeholder-slate-500 focus:border-indigo-500/60"
                  />
                </div>
                <div>
          ...
          {/* Footer Buttons matching website dark theme */}
